...
int main(void) {
    ...
    GLFWwindow* window =
        glfwCreateWindow(WIDTH, HEIGHT, "Shaders Lesson (C to frag uniform)", NULL, NULL);
    if (!window) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        const float green = ((float)sin(time) + 1.0f) / 2.0f;
        const int vertex_color_location = glGetUniformLocation(shader_program, "u_color");
